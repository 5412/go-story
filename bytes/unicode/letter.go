...
type RangeTable struct {
	R16         []Range16
	R32         []Range32
	LatinOffset int // number of entries in R16 with Hi <= MaxLatin1 16位中LATIN1编码的位置
}

// Range16 represents of a range of 16-bit Unicode code points. The range runs from Lo to Hi
// inclusive and has the specified stride 步副.
type Range16 struct {
	Lo     uint16
	Hi     uint16
	Stride uint16
...
// The constant 常数 UpperLower has an otherwise impossible delta value.
...
// Methods of SpecialCase customize 定做 (by overriding 覆盖) the standard mappings.
type SpecialCase []CaseRange

// BUG(r): There is no mechanism for full case folding, that is, for
// characters that involve multiple runes in the input or output.

// Indices 指标 into the Delta 增量 arrays inside CaseRanges for case mapping.
const (
	UpperCase = iota
	LowerCase
	TitleCase
...
// SimpleFold iterates 迭代 over Unicode code points equivalent under
