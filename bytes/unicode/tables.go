...
// Version is the Unicode edition from which the tables are derived 派生的.
const Version = "9.0.0"
...
var Categories = map[string]*RangeTable{
	...
	"Cf": Cf, // 格式字符
	"Co": Co, // 专用字符
	"Cs": Cs, // 指示字符是高代理项还是低代理项
	"L":  L,  // 字母
	"Ll": Ll, // 小写字母
	"Lm": Lm, // 修饰符字母
	"Lo": Lo, // 指示字符是字母，但它不是大写字母、小写字母、词首字母大写或修饰符字母。
	"Lt": Lt, // 指示字符是词首字母大写字母。
	"Lu": Lu, // 指示字符是大写字母。
	"M":  M,  //
	"Mc": Mc, // 指示字符是间距字符，这指示基字符的修改并影响该基字符的标志符号的宽度。
	"Me": Me, // 指示字符是封闭符号，封闭符号是非间距组合字符，它环绕直到基字符（并包括基字符）的所有前面的字符。
	"Mn": Mn, // 指示字符是非间距字符，这指示基字符的修改。
	"N":  N,  //
	"Nd": Nd, // 指示字符是十进制数字，即在范围 0 到 9 内。
	"Nl": Nl, // 指示字符是由字母表示的数字，而不是十进制数字，例如，罗马数字 5 由字母“V”表示。
	"No": No, // 指示字符是数字，但它既不是十进制数字也不是字母数字，例如分数 1/2。
	"P":  P,  //
	"Pc": Pc, // 指示字符是连接两个字符的连接符标点。
	"Pd": Pd, // 指示字符是短划线或连字符
	"Pe": Pe, // 指示字符是成对的标点符号（例如括号、方括号和大括号）之一的封闭字符
	"Pf": Pf, // 指示字符是封闭或后引号。
	"Pi": Pi, // 指示字符是开始或前引号。
	"Po": Po, // 指示字符是标点，但它不是连接符标点、短划线标点、开始标点、结束标点、前引号标点或后引号标点。
	"Ps": Ps, // 指示字符是成对的标点符号（例如括号、方括号和大括号）之一的开始字符。
	"S":  S,  //
	"Sc": Sc, // 指示字符是货币符号
	"Sk": Sk, // 指示字符是修饰符符号，这指示环绕字符的修改。例如，分数斜线号指示其左侧的数字为分子，右侧的数字为分母。
	"Sm": Sm, // 指示字符是数学符号，例如“+”或“=”。
	"So": So, // 指示字符是符号，但它不是数学符号、货币符号或修饰符符号。
	"Z":  Z,  //
	"Zl": Zl, // 指示字符用于分隔文本各行。
	"Zp": Zp, // 指示字符用于分隔段落。
	"Zs": Zs, // 指示字符是空白字符，它不具有标志符号，但不是控制或格式字符。
}
...
// Scripts is the set of Unicode script 脚本 tables.
